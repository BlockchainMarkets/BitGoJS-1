...
Wallet.prototype.prebuildAndSignTransaction = function(params, callback) {
  return co(function *() {
    ...
    const backupPublicKeys = this.baseCoin.keychains().get({ id: this._wallet.keys[1], reqId: params.reqId });
    const bitgoPublicKey = this.baseCoin.keychains().get({ id: this._wallet.keys[2], reqId: params.reqId });

    const [txPrebuild, userKeychain, backupKeychain, bitgoKeychain ] = yield Promise.all([txPrebuildQuery, userKeychainQuery, backupPublicKeys, bitgoPublicKey ]);
